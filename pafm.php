...
//set memory limits (because of various server configurations out there, wrap in try-catch)
try{ set_time_limit(3000); } catch (\Exception){}
try{ ini_set('max_execution_time', 3000); } catch (\Exception){}
try{ ini_set('memory_limit', '100M'); } catch (\Exception){}
...
if (count($allowed_IPs) !==0 && !in_array($_SERVER['REMOTE_ADDR'], $allowed_IPs)){
	die("Your IP: <b>".$_SERVER['REMOTE_ADDR'].'</b> is not added in allowed list.');
}
...
// https://github.com/ttodua/useful-php-scripts/blob/master/zip-folder.php
...
function doArchive($subject, $path){
	chdir(dirname(__file__));
	if (!empty($path))
	{
		$folderName = preg_replace("/%u([0-9a-f]{3,4})/i","&#x\\1;",urldecode($path)); 
		$folderName = html_entity_decode($folderName, ENT_SUBSTITUTE, 'UTF-8');
		//remove starting dot
		$folderName = substr($folderName,1);
		$zipFile = __DIR__. '/'.basename($folderName).'.zip';
		// delte previous action
		if (file_exists($zipFile)) {unlink($zipFile);}
		zip_folder(__DIR__.$folderName, $zipFile);
		// rename($zipFile, dirname($folderName).'/'.$zipFile );
		die("$zipFile successfully archived, refresh directory to see the added file.");
	}
}
...
function getDirs($path){
	...
	foreach ($dirContents['folders'] as $dirItem){
		...
		"\n\t" . '<a href="?do=archive&amp;path=' . (ROOT . $pathURL . '/' . $dirItemHTML) . '&amp;subject='.$dirItemURL.'&amp;nonce=' . $nonce.'" target="_blank" title="Zip '.$dirItemHTML.'" onclick="return confirm(\'This action will create '.removeQuotes($dirItem).'.zip file in the current directory\');" class="zip b">Zip</a>' .
